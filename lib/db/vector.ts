...
export class VectorNamespace<Metadata extends Record<string, unknown>> {
  ...
  async insert(docs: DocumentInterface<Metadata>[]) {
    try {
      ...
      const batchRequests = docChunks.map((chunk) =>
        this.caller.call(async () => this.namespace.upsert<any>(chunk)),
      )
  ...
  async query(
    ...
    k: number,
    filter?: string,
  ): Promise<[DocumentInterface<Metadata>, score: number][]> {
    try {
      ...
      return results.map(({ id, data, score, metadata }) => [
        { id: id as string, data: data!, metadata: metadata! },
        score,
      ])
    } catch (_err) {
