...
export class BillingPeriodModel extends DbModel {
  ...
  async ensureCurrent(
    billingId: string,
    config: Pick<BillingPeriodRecord, 'tier' | 'maxChatUsage'>,
  ) {
    const { id, parts } = this.getId(billingId)
    try {
      const [current] = await this.db.select().from(billingPeriods).where(eq(billingPeriods.id, id))
