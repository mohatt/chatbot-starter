...
export class ChatMessageModel extends DbModel {
  ...
  async findMany(
    ...
    try {
      const data = await this.db
        .select()
        .from(messages)
        .where(and(eq(messages.chatId, chatId), cursor ? lt(messages.id, cursor) : undefined))
        .limit(limit)
  ...
  async deleteMany(chatId: string, afterId?: string): Promise<number | null> {
    try {
      const { rowCount } = await this.db
        .delete(messages)
        .where(and(eq(messages.chatId, chatId), afterId ? gte(messages.id, afterId) : undefined))
      return rowCount
    } catch (_error) {
  ...
  private toDBMessages(chatId: string, uiMessages: ChatMessage[]) {
    return uiMessages
      .map(({ id, role, parts, metadata = {} }): ChatMessageRecordInput => {
